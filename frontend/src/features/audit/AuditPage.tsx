      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="flex items-center gap-1 text-[11px] font-bold px-2 py-0.5 rounded-full bg-indigo-950/80 text-indigo-300 border border-indigo-800/60 font-mono">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <div className="relative">
            ...
            <input
              ...
              className="text-xs pl-8 pr-3 py-2 rounded-xl bg-slate-900 border border-slate-800 text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500 w-44 sm:w-52 transition-colors"
            />
          </div>
              ...
              {loading ? (
                ...
                filteredEvents.map((ev) => (
                  <tr key={ev.id} className="hover:bg-slate-800/40 transition-colors group">
                    <td className="px-5 py-4 font-mono font-bold text-white">{ev.id.slice(0, 16)}...</td>
                    <td className="px-5 py-4">
                      <div className="font-bold text-slate-200">{formatAuditEventType(ev.event_type)}</div>
                      <div className="text-[10px] font-mono text-slate-500">{ev.event_type}</div>
                    </td>
                    <td className="px-5 py-4 font-mono text-indigo-400">{ev.recovery_case_id || '—'}</td>
                    <td className="px-5 py-4">
                      <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-slate-800 text-slate-300 border border-slate-700 font-mono">
                        {ev.actor}
                      </span>
                    </td>
