...
const NETWORK_NODES: NetworkNode[] = [
  {
    ...
    subtitle: 'Webhook ingestion & PII sanitization',
    color: '#0891B2',
    badgeBg: 'bg-[#ECFEFF]',
  ...
  },
  {
    ...
    subtitle: 'Dual-Brain strategy & confidence scoring',
    color: '#7C3AED',
    badgeBg: 'bg-[#F5F3FF]',
  ...
  },
  {
    ...
    subtitle: 'P0–P4 zero-tolerance hard limits',
    color: '#059669',
    badgeBg: 'bg-[#ECFDF5]',
  ...
  },
  {
    ...
    subtitle: 'Smart links & clearing house retries',
    color: '#3B5BDB',
    badgeBg: 'bg-[#EEF2FF]',
  ...
  },
  {
    ...
    subtitle: 'Automated invoice settlement',
    color: '#059669',
    badgeBg: 'bg-[#ECFDF5]',
    badgeBorder: 'border-[#A7F3D0]',
    icon: CheckCircle2,
...
export const RecoveryNetworkVisualizer: React.FC = () => {
  ...
  return (
    <div className="bg-white border border-[#E5E7EB] rounded-2xl p-6 shadow-sm space-y-5">
      ...
      {/* Selected Node Telemetry Inspector Box */}
      <motion.div
        key={activeNodeData.id}
        initial={reducedMotion ? {} : { opacity: 0, y: 4 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.18 }}
        className="p-4 rounded-xl bg-[#F7F9FC] border border-[#E5E7EB] flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-2xs"
      >
        <div className="flex items-center gap-3">
          <div
            className={`p-2 rounded-xl border shadow-2xs ${activeNodeData.badgeBg} ${activeNodeData.badgeBorder}`}
            style={{ color: activeNodeData.color }}
          >
            <activeNodeData.icon className="w-4 h-4" />
          </div>
          <div>
            <h4 className="text-xs font-bold text-[#111827]">{activeNodeData.title}</h4>
        ...
        <div className="flex items-center gap-3 pl-2 sm:border-l sm:border-[#E5E7EB]">
          <div className="text-right">
            <span className="text-[10px] uppercase font-bold text-[#64748B] block">Node Status</span>
            <span className="text-xs font-bold font-mono text-[#111827]">{activeNodeData.metricLabel}</span>
          </div>
          <span className="text-sm font-black font-mono" style={{ color: activeNodeData.color }}>
