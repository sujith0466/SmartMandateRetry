...
import { Inbox, Cpu, Sparkles, ShieldCheck, Zap, CheckCircle2, ArrowRight } from 'lucide-react';
...
const STEPS: Step[] = [
  ...
  {
    id: 2,
    title: '2. Context Evaluation',
    subtitle: 'Customer history & invoice risk extraction',
    badge: 'CONTEXT',
    icon: Cpu,
    color: '#7C3AED',
    badgeBg: 'bg-[#F5F3FF]',
    ...
    details: [
      'Extracts customer track record (12+ mos tenure, 95% historical recovery rate).',
      'Classifies payment failure reason across 14 failure taxonomy families.',
      'Evaluates invoice amount threshold (>₹10k escalation flag).',
    ],
    codeSample: '{\n  "customer_tenure_months": 14,\n  "historical_recovery_rate": 0.95,\n  "failure_family": "INSUFFICIENT_FUNDS",\n  "invoice_amount_inr": 4999.00\n}',
  },
  {
    id: 3,
    title: '3. AI Strategy Proposal',
    subtitle: 'Optimal channel & clearing window recommendation',
    badge: 'AI STRATEGY',
    icon: Sparkles,
    color: '#7C3AED',
    badgeBg: 'bg-[#F5F3FF]',
    badgeBorder: 'border-[#DDD6FE]',
    details: [
      'Gemini 2.0 Flash calculates optimal debit window at 06:00 IST for bank clearing liquidity.',
      'Generates model confidence score (91%) and feature attribution weight distribution.',
      'Proposes RETRY_SCHEDULED recovery action.',
    ],
    codeSample: '{\n  "ai_proposal": "RETRY_SCHEDULED",\n  "confidence": 0.91,\n  "optimal_window": "06:00:00+05:30",\n  "attribution": { "customer_tenure": +0.35, "insufficient_funds": +0.45 }\n}',
  },
  {
    id: 4,
    title: '4. Deterministic Safety Validation',
  ...
  {
    id: 5,
    title: '5. Multi-Channel Rail Dispatch',
...
export const LifecycleScrollSection: React.FC = () => {
  ...
  return (
    <section id="lifecycle" className="py-24 bg-[#FAF8F3] border-b border-[#E8E1D5] relative">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 space-y-16">
        {/* Header */}
        <div className="text-center max-w-3xl mx-auto space-y-3">
          <div className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full bg-[#ECFEFF] border border-[#A5F3FC] text-xs font-bold text-[#0891B2]">
            <Zap className="w-3.5 h-3.5" />
            <span>Interactive Lifecycle</span>
          </div>
          <h2 className="text-3xl sm:text-4xl font-black text-[#111827] tracking-tight font-sans">
            6 Stages of Autonomous Mandate Recovery
          </h2>
          <p className="text-sm sm:text-base text-[#64748B] font-medium leading-relaxed">
            From the instant a recurring mandate fails to settled bank reconciliation in 6 deterministic stages.
          </p>
        </div>
          ...
          {STEPS.map((s) => {
            ...
            return (
              <button
                ...
                className={`flex-1 min-w-[140px] flex items-center justify-center gap-2 py-3 px-3 rounded-xl text-xs font-bold transition-all relative ${
                  isSelected
                    ? 'bg-[#FAF8F3] text-[#111827] shadow-sm border border-[#CBD5E1]'
          ...
          >
            ...
            <div className="lg:col-span-6 space-y-6">
              ...
              <div className="flex items-center gap-3 pt-2">
                <button
                  onClick={() => setSelectedStep(selectedStep === 6 ? 1 : selectedStep + 1)}
                  className="inline-flex items-center gap-1.5 text-xs font-bold text-[#3B5BDB] hover:text-[#3048B8] transition-colors"
                >
                  <span>{selectedStep === 6 ? 'Restart Lifecycle' : 'Next Step'}</span>
                  <ArrowRight className="w-3.5 h-3.5" />
                </button>
              </div>
            </div>

            {/* Right Telemetry Code Box */}
